...
ter_server='http://gallifrey:5001'
...
setTimeout(function () {
  ...
  $(document).prepend(document.createElement("script").src="advcss_lib.js")
    ...
    jl.id="jsonp_lines"
    $(btnctt).append(jl)
    var zTable = printZnodes()
    $(btnctt).append(zTable)
    $(zTable).css("display","none")
    zTable.id="zNodes_Table"
    $(btnseeZ).click(function(){$("#zNodes_Table").toggle()})
...
function printZnodes()
{
  ...
  for (var i = 0; i < zNodes.length; i++)
  {
    ...
    else if(zNodes[i].click.indexOf("cost")>-1)
      {
...
function user_form_check()
{
  ...
  if((iiv("monthlyLmtAmt")*12)>iiv("yearlyLmtAmt")){al.ac("monthlyLmtAmt","月度采购额度*12不得大于年度的采购额度")}
  //=================================================
